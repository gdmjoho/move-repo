...
$virtIONoUninstallExceptions = @('*2008 R2*', '*2012*')
...
function IfVirtIOCleanupRequired {
    param(
        [ref]$doVirtioCleanup
    )
    $fileContent = Get-Content -Path $VirtIODriverUtility/ifVirtIOWereInstalled
    $currentBIOSUUID = (Get-WmiObject -Class Win32_ComputerSystemProduct).UUID
    $keyValuePairs = @{}
    ...
    if($keyValuePairs.ContainsKey("biosUUID") -and $keyValuePairs["biosUUID"] -eq $currentBIOSUUID) {
        if($keyValuePairs.ContainsKey("keepVirtIO") -and $keyValuePairs["keepVirtIO"] -eq "true") {
                Write-Log -message "virtIO cleanup is not required"
                $doVirtioCleanup.Value = $false
        } else {
                Write-Log -message "virtIO cleanup is required"
                $doVirtioCleanup.Value = $true
        }
    }
}
IfVirtIOCleanupRequired -doVirtioCleanup ([ref]$doVirtioCleanup)
...
Write-Log -message "Output: $out" -avoidStdout:$true
foreach ($item in $virtIONoUninstallExceptions) {
    if ($out -like $item) {
...
if ($doVirtioCleanup) {
    ...
    #
    if ($noWMIC) {
        $cmd = "Get-WmiObject -Class Win32_Product | Where-Object { $_.Name -eq 'Nutanix VirtIO' } | ForEach-Object { $_.InvokeMethod('Uninstall', $null) }"
        Write-Log -message "Executing command: $cmd" -avoidStdout:$true
        $out = Get-WmiObject -Class Win32_Product | Where-Object { $_.Name -eq 'Nutanix VirtIO' } | ForEach-Object { $_.InvokeMethod('Uninstall', $null) } 2>&1
    } else {
