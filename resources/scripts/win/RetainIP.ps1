...
$Code = {
    ...
    $WMIUTIL_CLUSTER_IPS_FILE = Join-Path $TempDir -ChildPath "cluster-ips.json"
    ...
    $WMIUTIL_CUSTOMIP_LOG = Join-Path $TempDir -ChildPath "wmi-net-util-customip-log.txt"
    $WMIUTIL_ROUTES_RESTORE_LOG = Join-Path $TempDir -ChildPath "wmi-net-util-routes-restore-log.txt"
    $WMIUTIL_STATIC_IPV6_CONFIG_DUMP = Join-Path $TempDir -ChildPath "wmi-static-ipv6-config-dump.txt"
    ...
    $MaxRetriesToFetchEnabledNics = 6
    $DelayToFetchEnabledNicsPerRetryInSec = 10
...
"@
    ...
    [bool] $IsCustomStaticIP = $false
    if ($PREP_CONF_FILE) {
        $IsCustomStaticIP = $true
    }
    if ($args[0] -like "source"){
        Write-Log -message  "Capturing the ip information in $TMPFILE"
        cmd.exe /c "$WMIUTIL -dump $TMPFILE -dumpStaticIpv6 $WMIUTIL_STATIC_IPV6_CONFIG_DUMP" 2>&1 | % { "$_" } | Out-File $WMIUTIL_DUMP_LOG
        $ExitCode = $LASTEXITCODE
        if ($ExitCode -ne 0) {
            Write-Log -message  "wmi-net-util command failed"
            Write-Output "wmi-net-util command failed" > $RESULT_FILE
            exit $ExitCode
        }
        Write-Log -message  "Capturing the static route information in $ROUTEFILE"
        cmd.exe /c "$WMIUTIL -dumpRoutes $ROUTEFILE" 2>&1 | % { "$_" } | Out-File $WMIUTIL_DUMP_LOG
        $ExitCode = $LASTEXITCODE
        if ($ExitCode -ne 0) {
            Write-Log -message  "static-route dump failed"
            Write-Output "static-route dump failed" > $RESULT_FILE
            exit $ExitCode
        }
        Write-Log -message  "Capturing the cluster Ips information in $WMIUTIL_CLUSTER_IPS_FILE"
        $clusterIpAddress = Get-ClusterResource | Where-Object { $_.ResourceType -eq 'IP Address' -or $_.ResourceType -eq 'IPv6 Address' } | ForEach-Object {
            $_ | Get-ClusterParameter |
            Where-Object { $_.Name -eq 'Address' } |
            Select-Object @{ Name = "IP"; Expression = { $_.Value } }
        }
        if ($? -eq $false) {
            # Ignore the error as command may fail if cluster isn't configured.
            Write-Log -message  "cluster ips dump failed $ExitCode"
            Write-Output "cluster ips dump failed" > $RESULT_FILE
        }
        else {
            $clusterIpAddress | ConvertTo-Json -Compress | Set-Content -Path $WMIUTIL_CLUSTER_IPS_FILE
        }
        $ExitCode = $LASTEXITCODE
        ...
        if ($SkipRemoveGhostNIC -eq $false) {
            Write-Log -message  "Removing ghost NICs completely"
            Remove-Ghost-Nics
        }
        Get-Date >> $WMI_NIC_DUMP_LOG

        if ($IsCustomStaticIP) {
            Get-Date >> $WMIUTIL_CUSTOMIP_LOG
            Write-Log -message  "Applying the ip information from $PREP_CONF_FILE"
            cmd.exe /c "$WMIUTIL -isCustomStaticIP -customConfFile $PREP_CONF_FILE -restoreStatic $TMPFILE -excludeClusterIps $WMIUTIL_CLUSTER_IPS_FILE -restoreStaticIpv6 $WMIUTIL_STATIC_IPV6_CONFIG_DUMP -maxRetriesToFetchEnabledNics $MaxRetriesToFetchEnabledNics -delayToFetchEnabledNicsPerRetryInSec $DelayToFetchEnabledNicsPerRetryInSec" 2>&1 | % { "$_" } | Out-File $WMIUTIL_CUSTOMIP_LOG
            $ExitCode = $LASTEXITCODE
            if ($ExitCode -ne 0) {
                Write-Log -message  "Custom network configuration failed"
                Write-Output "Custom network configuration failed" > $RESULT_FILE
                exit $ExitCode
            }
        } else {
            Get-Date >> $WMIUTIL_RESTORE_LOG
            Write-Log -message  "Applying the ip information from $TMPFILE"
            cmd.exe /c "$WMIUTIL -restoreStatic $TMPFILE -excludeClusterIps $WMIUTIL_CLUSTER_IPS_FILE -restoreStaticIpv6 $WMIUTIL_STATIC_IPV6_CONFIG_DUMP -maxRetriesToFetchEnabledNics $MaxRetriesToFetchEnabledNics -delayToFetchEnabledNicsPerRetryInSec $DelayToFetchEnabledNicsPerRetryInSec" 2>&1 | % { "$_" } | Out-File $WMIUTIL_RESTORE_LOG
            $ExitCode = $LASTEXITCODE
            if ($ExitCode -ne 0) {
                Write-Log -message  "Restore network configuration failed"
                Write-Output "Restore network configuration failed" > $RESULT_FILE
                exit $ExitCode
            }
            if ( $(Get-Content $ROUTEFILE) -ne "[]" ) {
                Start-Sleep -s 120
                Write-Log -message  "Applying the static route information from $ROUTEFILE"
                cmd.exe /c "$WMIUTIL -restoreRoutes $ROUTEFILE -sourceNICs $TMPFILE -maxRetriesToFetchEnabledNics $MaxRetriesToFetchEnabledNics -delayToFetchEnabledNicsPerRetryInSec $DelayToFetchEnabledNicsPerRetryInSec"  2>&1 | % { "$_" } | Out-File $WMIUTIL_ROUTES_RESTORE_LOG
                $ExitCode = $LASTEXITCODE
                if ($ExitCode -ne 0) {
                    Write-Log -message  "static-route restoration failed"
                    Write-Output "static-route restoration failed" > $RESULT_FILE
                    exit $ExitCode
                }
            }
        }
        Write-Log -message  "Reregistering DNS"
        cmd.exe /c "ipconfig /flushdns && ipconfig /registerdns" >> $WMIUTIL_RESTORE_LOG
        $ExitCode = $LASTEXITCODE
        if ($ExitCode -ne 0) {
            Write-Log -message  "Reregistering DNS failed moving on"
...
if ($Job.State -ne "Completed") {
    ...
    Write-Log-Global -message "Retain IP script has not completed within timelimit"
    Write-Log-Global -message "Partial Job Output = '$JobOutput'"
    exit 1
} else {
    Write-Log-Global -message "Retain IP script has completed execution within Time Limit"
    $JobOutput = $Job | Receive-Job
